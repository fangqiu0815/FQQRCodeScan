...
@implementation FQQRCodeScanView
{
    BOOL needStop;
}
...
- (instancetype)initWithFrame:(CGRect)frame style:(NSString *)style
{
    if (self = [super initWithFrame:frame]) {
        ...
        _scanType = FQQRCodeScanTypeQrCode;
        self.heightScale = 1;
...
- (void)setScanType:(FQQRCodeScanType)scanType
{
    _scanType = scanType;
    if (_scanType == FQQRCodeScanTypeBarCode) {
        self.heightScale = 3;
...
- (void)fq_startAnimation
{
    ...
    [UIView animateWithDuration:1.5 animations:^{
        ...
        _lineImageView.alpha = 0;
        _lineImageView.frame = initFrame;
        [weakSelf performSelector:@selector(fq_startAnimation) withObject:nil afterDelay:0.3];
    }];
}
